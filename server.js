#!/usr/bin/env node
/**
 * DroidMC - full Minecraft server panel backend for Termux.
 */
...
const UI_DIR = path.join(HOME, 'DroidMC');
...
function getSessionFromRequest(req) {
  pruneSessions();
  const token = parseCookies(req.headers.cookie || '').droidmc_session;
  if (!token) {
    return null;
  }
...
function setSessionCookie(res, token) {
  res.setHeader(
    'Set-Cookie',
    `droidmc_session=${token}; Max-Age=${Math.floor(SESSION_TTL_MS / 1000)}; Path=/; HttpOnly; SameSite=Strict`,
  );
}

function clearSessionCookie(res) {
  res.setHeader('Set-Cookie', 'droidmc_session=; Max-Age=0; Path=/; HttpOnly; SameSite=Strict');
}
...
async function httpsGetRaw(url, depth = 0) {
  return new Promise((resolve, reject) => {
    ...
    mod.get(url, { headers: { 'User-Agent': 'DroidMC/2.4 (termux-panel)' } }, (res) => {
...
function downloadFileWithProgress(url, outPath) {
  return new Promise((resolve, reject) => {
    const doGet = (currentUrl, depth = 0) => {
      ...
      mod.get(currentUrl, { headers: { 'User-Agent': 'DroidMC/2.4' } }, (response) => {
